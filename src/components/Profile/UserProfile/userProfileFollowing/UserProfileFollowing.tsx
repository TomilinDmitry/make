...
export const UserProfileFollowing = () => {
  ...
  const usersId = following?.results.map((el) => el.user_id);
  ...
  const usersWithCounters: Record<
    string,
    { total_views: number; count_lessons: number; user_id: string }
  > = {};
            ...
            <div className={s.box}>
              ...
              <div className={s.userSmall}>
                <Link
                  ...
                  <UserCardSmall userData={user} totalViews={counter.total_views} countLessons={counter.count_lessons} />
                </Link>
              </div>
            </div>
          );
